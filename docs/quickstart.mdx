...
To upgrade Smithers to the latest version:
...
This launches the interactive agent. Same API as [OpenCode](https://opencode.ai) since Smithers wraps it.
...
function Workflow() {
  const phase = db.state.get('phase') ?? 'research';
  
  return (
    <SmithersProvider db={db} executionId={executionId}>
      <Ralph id="workflow" condition={() => phase !== 'done'} maxIterations={10}>
        <Phase name="research" onComplete={() => db.state.set('phase', 'implement')}>
          <Step name="analyze">
            <Claude>Analyze the codebase structure.</Claude>
          </Step>
        </Phase>

        <Phase name="implement" onComplete={() => db.state.set('phase', 'review')}>
          <Step name="code">
        ...
        <Phase name="review" onComplete={() => db.state.set('phase', 'done')}>
          <Step name="review">
...
**`SmithersProvider`**: Root component. Provides database and execution context.

**`Ralph`**: Runs the reactive render loop for iterative workflows.
