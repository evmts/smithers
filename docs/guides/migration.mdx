...
result = chain.run(question="What is SolidJS?")
```
...
const result = await executePlan(() => <Assistant question="What is SolidJS?" />)
```
...
function SequentialAgent(props) {
  const [phase, setPhase] = createSignal('research')
  const [research, setResearch] = createSignal(null)
    ...
    if (p === 'research') {
      return (
        <Claude onFinished={(data) => { setResearch(data); setPhase('write') }}>
          Research this topic: {props.topic}
        </Claude>
      )
    }
...
function TaskAgent() {
  ...
  const completeTask = (task, result) => {
    setState(s => ({
      ...
      completed: [...s.completed, { task, result }]
    }))
  }

  const addTasks = (newTasks) => {
    setState('tasks', prev => [...prev, ...newTasks])
  }
    ...
    return (
      ...
      >
        <Phase name="execute">
          ...
          Completed so far:
          {state.completed.map(c => `- ${c.task}: ${c.result.summary}`).join('\n')}
        </Phase>
...
function Researcher(props) {
  return (
    <Claude tools={[searchTool]} onFinished={props.onComplete}>
...
function Writer(props) {
  return (
    <Claude onFinished={props.onComplete}>
      ...
      <Phase name="write">
        Create content based on: {JSON.stringify(props.research)}
      </Phase>
    </Claude>
  )
}
  ...
  return (
    <Claude onFinished={props.onComplete}>
      <Phase name="summarize">
        Summarize: {props.text}
      </Phase>
    </Claude>
  )
}

// Usage - composable like Solid components
function MyAgent(props) {
  ...
  return () => {
    const s = summary()
    if (!s) {
      return <Summarizer text={props.document} onComplete={setSummary} />
    }

    return (
      <Claude>
        Based on this summary: {s}
        Take next action...
      </Claude>
    )
  }
...
| Memory | Solid Store or Signals |
| Prompt Template | JSX with interpolation |
| Sequential Chain | `createSignal` phase transitions |
...
// After (1.0)
import { Claude, renderPlan, executePlan } from '@evmts/smithers'
```
...
**Solid Version**: Smithers requires Solid 1.9+ for universal renderer features.
...
<Claude>
  You are a {props.role}. Your task is to {props.task}.
  Input: {JSON.stringify(props.input)}
</Claude>

// Conditional sections
<Claude>
  {props.context && <Phase name="context">{props.context}</Phase>}
  <Phase name="main">{props.mainPrompt}</Phase>
</Claude>

// Dynamic lists
<Claude>
  <Phase name="review">
    {props.files.map(f => <Step key={f}>Review: {f}</Step>)}
  </Phase>
</Claude>
```
...
// Complex state with Solid Store
const [state, setState] = createStore({
  phase: 'start',
  data: {}
})
