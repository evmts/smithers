...
function BranchingAgent({ input }) {
  const [phase, setPhase] = createSignal<'analyze' | 'simple' | 'complex' | 'done'>('analyze')
  const [complexity, setComplexity] = createSignal<string | null>(null)
...
function ConditionalWorkflow({ config }) {
  const [phase, setPhase] = createSignal('init')
...
function FanOutFanIn({ items }) {
  const [phase, setPhase] = createSignal<'process' | 'aggregate' | 'done'>('process')
...
function RateLimitedExecutor({ items, maxConcurrent = 3 }) {
  const [pending, setPending] = createSignal(items)
  const [running, setRunning] = createSignal<string[]>([])
  const [completed, setCompleted] = createSignal<Record<string, any>>({})
...
function GracefulAgent({ sources }) {
  const [results, setResults] = createSignal<Record<string, any>>({})
  const [errors, setErrors] = createSignal<Record<string, Error>>({})
  const [phase, setPhase] = createSignal<'fetch' | 'process' | 'done'>('fetch')
