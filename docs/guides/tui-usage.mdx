...
The TUI uses the same Solid renderer architecture as Smithers' core renderer:

- **SmithersNode tree**: Directly visualized
- **Real-time updates**: `onFrameUpdate` callback provides tree snapshots
- **Fine-grained reactivity**: Efficient updates without full tree re-renders

### Performance

The TUI architecture is designed for performance:

- **Update frequency**: Once per Ralph Wiggum loop frame
- **Minimal overhead**: Lightweight state management via Solid signals

## Advanced Usage

### Programmatic TUI

You can hook into the execution loop to build custom visualizations:

```typescript
import { executePlan } from '@evmts/smithers'

// Execute with custom frame handling
const result = await executePlan(() => <MyAgent />, {
  onFrameUpdate: (tree, frame) => {
    // Render tree to your custom UI
    myCustomRenderer.update(tree, frame)
  },
})
```
