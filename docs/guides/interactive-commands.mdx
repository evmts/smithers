...
│     execution: { status: "complete", result: "Implementation plan created" }
...
# Output:
# ✓ Injected context for next Claude node
...
# Output:
# 🛑 Execution aborted: Found critical bug, stopping execution
```
...
```typescript
import { executePlan, ExecutionController } from 'smithers'
...
Commands are not saved to history by default. Use your terminal's history feature (↑ arrow) to repeat commands.
