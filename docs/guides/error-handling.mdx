...
```tsx
import { useSmithers, If, Stop, Claude, useQueryValue } from "smithers-orchestrator";

function RetryWorkflowBody() {
  const { db } = useSmithers();
  const { data: attemptsVal } = useQueryValue<number>(
    db.db, "SELECT CAST(value AS INTEGER) FROM state WHERE key = 'attempts'"
  );
  const attempts = attemptsVal ?? 0;
  const { data: lastError } = useQueryValue<string>(
    db.db, "SELECT value FROM state WHERE key = 'lastError'"
  );
...
<SmithersProvider
  ...
  stopConditions={[
    ...
    { type: "report_severity", message: "Critical report found" },
    { type: "custom", fn: (ctx) => ctx.totalTokens > 40000 && ctx.elapsedTimeMs > 60000, message: "Budget exceeded" },
  ]}
/>
```
...
```tsx
import { useSmithers, If, Claude, Human, useQueryValue } from "smithers-orchestrator";

function ResilientWorkflowBody() {
  const { db } = useSmithers();
  const { data: approach } = useQueryValue<string>(
    db.db, "SELECT value FROM state WHERE key = 'approach'"
  );
