...
export function RetryWorkflow({ db, executionId }) {
  return (
...
function ResilientWorkflowBody() {
  ...
  const { data: approachJson } = useQueryValue<string>(
    reactiveDb,
    "SELECT value FROM state WHERE key = 'approach'"
  );
  const approach = approachJson ? JSON.parse(approachJson) : "full";
...
export function ResilientWorkflow({ db, executionId }) {
  return (
    <SmithersProvider db={db} executionId={executionId} maxIterations={5}>
      <ResilientWorkflowBody />
    </SmithersProvider>
  );
}
```
...
function RobustWorkflowBody() {
  ...
  const { data: errorsJson } = useQueryValue<string>(
    reactiveDb,
...
export function RobustWorkflow({ db, executionId }) {
  return (
    <SmithersProvider
      db={db}
      executionId={executionId}
