...
sqlite3 .smithers/my-workflow.db "SELECT * FROM phases ORDER BY created_at"
```

### View Step Progress

```bash
sqlite3 .smithers/my-workflow.db "SELECT * FROM steps ORDER BY created_at"
```

### View Running Tasks

```bash
sqlite3 .smithers/my-workflow.db "SELECT * FROM tasks WHERE status = 'running'"
```

### View State History

```bash
sqlite3 .smithers/my-workflow.db "SELECT * FROM state ORDER BY updated_at DESC"
```
...
sqlite3 .smithers/db "SELECT * FROM tasks WHERE status = 'running'"

# Check last state change
sqlite3 .smithers/db "SELECT * FROM state ORDER BY updated_at DESC LIMIT 5"
```
...
```bash
# Check step status
sqlite3 .smithers/db "SELECT * FROM steps WHERE phase_id = '<phase-id>'"
```
