...
1. **Render**: JSX components render to PluNode tree
...
**Problem**: Parallel callbacks capture stale state

```tsx
// BUG: Each callback sees results = {}
function BadParallel({ items }) {
  const [results, setResults] = useState({})

  return (
    <>
      {items.map(item => (
        <Subagent key={item.id}>
          <Claude onFinished={(r) => setResults({ ...results, [item.id]: r })}>
            Process: {item}
          </Claude>
        </Subagent>
      ))}
...
**Solution**: Use Zustand or functional updates

```tsx
// CORRECT: Functional update gets latest state
...
function RobustAgent() {
  ...
  if (error) {
    return (
      <Claude>
        Previous operation failed: {error.message}
        Attempting recovery...
      </Claude>
    )
  }
...
```tsx
function DebugAgent() {
  const { phase, data } = useStore()

  // Log on every render
  console.log('[DebugAgent] Render:', { phase, dataKeys: Object.keys(data) })
...
Smithers uses real React, so React DevTools work:
