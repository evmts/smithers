...
smithers db execution <execution-id>                # View details
smithers db state --execution-id <execution-id>        # View state
```
...
sqlite3 .smithers/my-workflow.db "SELECT * FROM state ORDER BY updated_at DESC LIMIT 5"
```
...
Correlate with:
```tsx
<Claude onFinished={(r) => console.log("Output:", r.output)}>
```
...
Debug: `SELECT * FROM steps WHERE phase_id = '<phase-id>'`
