    ...
    return (
      <Claude
        ...
        onFinished={(result) => {
          ...
          // Expected format: filename followed by processed content
          const processed: Record<string, string> = {}
          const resultText = result.text

          // Simple parsing: look for file paths followed by content
          files.forEach(file => {
            const filename = file.split('/').pop()
            const filePattern = new RegExp(`${filename}\\s*\\n([\\s\\S]*?)(?=\\n\\S+\\.md|$)`)
      ...
      >
        ...
        Output each file's processed content with the filename as a header.
      </Claude>
    )
  }
