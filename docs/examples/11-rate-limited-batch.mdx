...
```tsx
import { executePlan, ClaudeProvider, Claude } from '@evmts/smithers'
  ...
  return (
    <ClaudeProvider
      ...
      onRateLimited={(reason, waitMs) => {
        console.log(`⏸️  Rate limited (${reason}), waiting ${waitMs}ms`)
      }}
      onUsageUpdate={(stats) => {
        console.log(
          `📊 Progress: ${stats.requestCount}/${items.length} ` +
...
📊 Progress: 10/100 ($0.0045)
📊 Progress: 20/100 ($0.0089)
⏸️  Rate limited (rpm), waiting 1234ms
📊 Progress: 30/100 ($0.0134)
✓ Processed: item-001
✓ Processed: item-002
...
📊 Progress: 100/100 ($0.0445)
...
```
100 items × 500 input tokens × $0.000003/token = $0.15
100 items × 200 output tokens × $0.000015/token = $0.30
