...
function GitHelper(props: {
  ...
  const [state, setState] = createStore<GitHelperState>({
    command: (props.operation as GitHelperState['command']) || 'status',
    result: null,
  })
    ...
    if (state.command === 'status') {
      return (
        <Claude
          allowedTools={['Bash']}
          onFinished={(result) => {
            setState({ result: result.text, command: 'done' })
          }}
        >
