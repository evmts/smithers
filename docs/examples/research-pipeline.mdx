...
description: "Multi-phase research agent using Zustand for state management"
---
...
3. When `onFinished` fires, it may update state (Solid Signals/Store)
...
## State Management with Solid Store
...
const actions = {
  setSources: (sources) => setState({ sources }),
  setAnalysis: (analysis) => setState({ analysis }),
  nextPhase: () => {
    const transitions = { gather: 'analyze', analyze: 'report', report: 'done' }
    setState('phase', transitions[state.phase])
  },
}
```
...
1. Reads current state from Store
...
function GatherPhase(props) {
  return (
    ...
    >
      <Phase name="gather">
        <Step>Search for sources about: {props.topic}</Step>
      </Phase>
    </Claude>
  )
}
...
```tsx
function ResearchPipeline(props) {
  // Use a derived closure
  return () => {
    switch (state.phase) {
      case 'gather':
        return <GatherPhase topic={props.topic} />
      case 'analyze':
        return <AnalyzePhase />
      case 'report':
        return <ReportPhase topic={props.topic} />
      case 'done':
...
Smithers recommends Solid Stores over simple signals for multi-phase agents:

1. **External state** - Solid stores state outside the component tree, making it accessible from `onFinished` callbacks
2. **Predictable updates** - Direct mutations via path syntax
3. **Debugging** - Easy to inspect full state at any point
4. **Testability** - Can reset/mock store state in tests

## Next Steps

See [Parallel Research](/examples/parallel-research) to learn about parallel execution with Subagent.
