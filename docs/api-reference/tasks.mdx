...
|--------|-----------|-------------|
| `start` | `(componentType: string, componentName?: string, options?: { scopeId?: string \| null }) => string` | Start task |
...
| `withTask` | `<T>(type: string, name: string \| undefined, options: { scopeId?: string \| null } \| undefined, fn: () => T \| Promise<T>) => Promise<T>` | Execute within task lifecycle |
