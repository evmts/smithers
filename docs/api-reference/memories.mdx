...
```tsx
// All facts
const facts = await db.memories.list({ category: "fact" });

// Project-scoped preferences
const prefs = await db.memories.list({ category: "preference", scope: "project" });

// All memories, limited
const all = await db.memories.list({ limit: 100 });
```
