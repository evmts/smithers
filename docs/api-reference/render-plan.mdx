---
title: renderPlan
description: Render a Solid component to XML plan without execution
---
...
```typescript
function renderPlan(element: () => JSX.Element): Promise<string>
```

## Parameters

<ParamField path="element" type="() => JSX.Element" required>
  A function that returns the JSX element to render. Typically `() => <Agent />`.
</ParamField>
...
```tsx
import { renderPlan, Claude, Phase, Step } from '@evmts/smithers'

function MyAgent(props) {
  return (
    <Claude tools={[webSearch]}>
      <Phase name="research">
        <Step>Search for {props.topic}</Step>
