...
Your orchestrating agent writes React code that spawns and controls other agents. The plan is the code. The code is the plan. [Let your agent write agents](/harness-integration#claude-code-plugin).
...
Smithers is a React framework for coding agents. Write the plan in any coding harness, then monitor your agents as they execute it. The plan evolves reactively based on agent output.

By evolving a declarative plan rather than wiring up every agent interaction, Smithers excels at:

1. One-time agents — accomplish a task and discard
2. Long-running agents — run for days or weeks
...
Most multi-agent frameworks fail. They add coordination overhead that costs more than it saves—you end up worse than a simple while loop. Smithers avoids this by making the plan declarative and the state durable. React's functional nature makes even complex multiagent setups easy to modularize and easy to reason about.
