---
title: Introduction
description: Build AI agents the way you build user interfaces - with SolidJS
---

# Welcome to Smithers

Smithers is a declarative framework for building AI agents using SolidJS and JSX. Build complex agent workflows from simple, reusable components - the same way you build user interfaces.

## Features

- 💳 **Use your Claude Code subscription** - Run agents with your existing Claude Code subscription
- ▲ **First-class Vercel AI SDK support** - Integrate seamlessly with Vercel AI SDK tools and patterns
- 🔷 **Zod schema validation** - Define structured outputs with Zod schemas
- 📝 **JSX/TSX & MDX support** - Author agents in `.tsx` or `.mdx` files
- ⚛️ **SolidJS state management** - Use signals and stores to control agent flow
...
- 🛠️ **Built-in tools** - Read, Edit, Bash, Glob, Grep, and more via Claude Agent SDK
...
```bash
# Run a TSX agent
npx @evmts/smithers run agent.tsx

# Run an MDX agent
npx @evmts/smithers run agent.mdx

# Preview the plan without executing
npx @evmts/smithers plan agent.tsx

# Auto-approve execution (skip preview)
npx @evmts/smithers run agent.tsx --auto-approve
```
...
import { executePlan, Claude, Phase, Step, Human, Stop } from '@evmts/smithers'
...
function FeatureWorkflow(props) {
  ...
  const nextPhase = () => {
    const phases: WorkflowPhase[] = ['prompt-input', 'research', 'planning', 'plan-review', 'poc', 'poc-analysis', 'refined-review', 'api-impl', 'test-impl', 'test-verify', 'implementation', 'done']
  ...
  return () => {
    switch (state.phase) {
      case 'prompt-input':
        return (
          <Human message="Review feature request" onApprove={() => { setState('prompt', props.prompt); nextPhase() }} onReject={() => setState('phase', 'cancelled')}>
      ...
      case 'plan-review':
        return <Human message="Review plan" onApprove={() => nextPhase()} onReject={() => setState('phase', 'cancelled')} />
      ...
      case 'poc-analysis':
        return (
          <Claude maxThinkingTokens={16000} onFinished={(result) => { setState('refinedPlan', result.refinedPlan); nextPhase() }}>
...
<CardGroup cols={2}>
  ...
  <Card title="SolidJS State Drives Behavior" icon="arrows-spin">
    Use signals and stores to control agent flow. State changes trigger re-renders.
  </Card>
