...
```tsx
import { ClaudeCli } from '@evmts/smithers'
...
Use Solid state to control the workflow:

```tsx
import { createStore } from 'solid-js/store'

function DeveloperAgent(props) {
  const [state, setState] = createStore({
    phase: 'research',
    findings: '',
  })

  const setFindings = (findings) => setState({ findings, phase: 'implement' })

  return () => {
    if (state.phase === 'research') {
      return (
        <ClaudeCli onFinished={setFindings}>
          Research the codebase to understand how to: {props.task}
    ...
    return (
      <ClaudeCli onFinished={() => console.log('Done!')}>
        Implement the following task: {props.task}
...
```tsx
function CodeReviewAgent(props) {
  return (
    <ClaudeCli
