...
function ResearchAgent(props) {
  return (
    <Claude tools={[webSearch, filesystem]}>
      <Phase name="research">
        <Step>Search for academic papers on {props.topic}</Step>
...
Use Solid's `<Show>` component or standard boolean logic to include or exclude phases:

```tsx
function FlexibleAgent(props) {
  return (
    <Claude tools={[filesystem]}>
      <Phase name="implement">
        <Step>Implement {props.task}</Step>
...
```tsx
function SecurityReviewPhase(props) {
  return (
    <Phase name="security-review">
      <Step>Check for injection vulnerabilities in {props.focus}</Step>
...
function TestingPhase(props) {
  return (
    <Phase name="testing">
      <Step>Write unit tests (target {props.coverage}% coverage)</Step>
...
// Usage
function SecureFeature(props) {
  return (
    <Claude tools={[filesystem, terminal]}>
      <Phase name="implement">
        <Step>Implement {props.feature}</Step>
      </Phase>

      <SecurityReviewPhase focus={props.feature} />
