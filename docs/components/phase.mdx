...
<Note>
Phase and Step components require `<Ralph>` or `<While>` for multi-iteration workflows. Without a loop wrapper, they execute once and stop. See [Ralph](/components/ralph).
</Note>
...
<ParamField path="skipIf" type="() => unknown">
  ...
  const { data: hasTests } = useQueryValue<string>(
    reactiveDb, "SELECT value FROM state WHERE key = 'has_tests'"
  )
...
// [{ id: "p1", name: "Research", status: "completed" }, ...]
```
