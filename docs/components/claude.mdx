...
<ParamField path="tools" type="(string | Tool | MCPServer)[]">
  Custom tools to provide to Claude. Can be tool names (strings), Tool objects, or MCPServer instances.
...
<ParamField path="permissionMode" type="'default' | 'acceptEdits' | 'plan' | 'dontAsk' | 'bypassPermissions'">
...
<ParamField path="mcpConfig" type="string">
  ...
  ```tsx
  <Claude mcpConfig="./mcp.json">
    Use MCP servers defined in config.
  </Claude>
  ```
</ParamField>

<ParamField path="outputFormat" type="'text' | 'json' | 'stream-json'" default="text">
  Output format for Claude's response.

  ```tsx
  <Claude outputFormat="json">Return structured data.</Claude>
  ```
</ParamField>
...
<ParamField path="continueConversation" type="boolean">
  ...
  ```tsx
  <Claude continueConversation>Follow up on previous task.</Claude>
  ```
</ParamField>
...
<ParamField path="resumeSession" type="string">
  ...
  ```tsx
  <Claude resumeSession="session-abc123">Continue this session.</Claude>
  ```
</ParamField>
...
<ParamField path="useSubscription" type="boolean" default="true">
...
<ParamField path="middleware" type="SmithersMiddleware[]">
  ...
  ```tsx
  <Claude middleware={[loggingMiddleware, rateLimitMiddleware]}>
...
<ParamField path="tailLogCount" type="number" default="10">
  ...
  ```tsx
  <Claude tailLogCount={20}>Show more log entries.</Claude>
  ```
</ParamField>
...
<ParamField path="tailLogLines" type="number" default="10">
  ...
  ```tsx
  <Claude tailLogLines={5}>Shorter log entries.</Claude>
  ```
</ParamField>
...
<ParamField path="retryDelayMs" type="number" default="250">
  ...
  ```tsx
  <Claude retryDelayMs={500} maxRetries={3}>Task with slower retries.</Claude>
  ```
</ParamField>
...
// Full edit access
<Claude
  allowedTools={["Read", "edit_file", "create_file", "Bash"]}
  ...
  return (
    <>
      ...
      <If condition={phase === "implement"}>
        <Claude
          allowedTools={["edit_file", "create_file"]}
...
<AccordionGroup>
  ...
  <Accordion title="Use allowedTools to restrict access">
    ...
    <Claude allowedTools={["Read", "edit_file", "create_file"]}>
    ```
  </Accordion>
