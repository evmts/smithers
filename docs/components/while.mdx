...
```tsx
function RefineImplementationLoop() {
  const { db } = useSmithers()
  ...
  return (
    <While
      ...
      condition={async () => {
        const quality = db.state.get<{ score: number }>('codeQuality')
        ...
        <Step name="measure">
          <Claude
            onFinished={(result) => {
              ...
              db.state.set('codeQuality', { score })
            }}
          >
...
<While
  id="wait-deployment"
  condition={async () => {
    const start = db.state.get('deploymentStartTime')
