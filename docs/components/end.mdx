...
<ParamField path="summary" type="EndSummary | (() => EndSummary | Promise<EndSummary>)" required>
  ...
  // Dynamic summary
  <End summary={() => {
    const result = db.state.get("result");
    return {
  ...
  return (
    <End
      summary={() => {
        ...
        const iterations = db.state.get("iterations");
...
function WorkflowWithEndBody() {
  ...
  const phase = phaseJson ? JSON.parse(phaseJson) : "working";
...
export function WorkflowWithEnd({ db, executionId }) {
  return (
...
const row = db.db.queryOne<{
  end_summary: string | null;
  end_reason: string | null;
  exit_code: number;
}>("SELECT end_summary, end_reason, exit_code FROM executions WHERE id = ?", [executionId]);

const endSummary = row?.end_summary ? JSON.parse(row.end_summary) : null;
console.log(endSummary);        // The EndSummary object
console.log(row?.end_reason);   // "success", "failure", etc.
console.log(row?.exit_code);    // 0, 1, etc.
```
