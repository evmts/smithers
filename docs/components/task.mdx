...
function TaskTracker() {
  ...
  const { data: tasksVal } = useQueryValue<string>(
    db.db,
    "SELECT value FROM state WHERE key = 'tasks'"
  );
  const tasks: Array<{id: number, name: string, done: boolean}> = tasksVal 
    ? JSON.parse(tasksVal) 
    : [
...
function PersistentTasks() {
  ...
  const tasks: Array<{name: string, done: boolean}> = tasksVal ? JSON.parse(tasksVal) : [];
...
function TaskDrivenWorkflowBody() {
  const { db } = useSmithers();
  ...
  const currentTaskIndex = indexVal ? JSON.parse(indexVal) : 0;
