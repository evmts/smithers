...
```tsx
import { Claude, OutputFormat } from '@evmts/smithers'
...
```tsx
// output-formats.tsx
import { OutputFormat } from '@evmts/smithers'
...
export function CodeOnlyOutput(props) {
  return (
    <OutputFormat>
      Return only the {props.language} code.
