...
```tsx
import { Worktree, Claude } from '@evmts/smithers'
...
function ReviewAgent(props) {
  ...
  return (
    <Worktree
      path={`./worktrees/pr-${props.prNumber}`}
      branch={`pr/${props.prNumber}`}
      baseBranch="main"
      cleanup={!approved()}
    >
      <Claude
...
function RefactorExperiment(props) {
  const [results, setResults] = createStore([])

  return (
    <>
      {props.approaches.map((approach, i) => (
        <Worktree
          key={i}
      ...
      {results.length === props.approaches.length && (
        <Claude>
...
function CrossVersionTest(props) {
  const [testResults, setTestResults] = createStore({})
