...
function ParallelAnalysis(props) {
  const [results, setResults] = createStore<Record<string, any>>({})

  return () => {
    const pendingFiles = props.files.filter(f => !results[f])
        ...
        <>
          {pendingFiles.map(file => (
            <Subagent key={file} name={`analyzer-${file}`}>
              <Claude
                tools={[filesystem]}
                onFinished={(analysis) => setResults(file, analysis)}
              >
...
```tsx
function ResearchDepartment(props) {
  return (
    <>
      {props.topics.map(topic => (
...
function ResilientParallel(props) {
  const [results, setResults] = createStore({})
  const [errors, setErrors] = createStore({})

  return () => {
    const pending = props.tasks.filter(t => !results[t.id] && !errors[t.id])
        ...
        <>
          {pending.map(task => (
            <Subagent key={task.id} name={`worker-${task.id}`}>
              <Claude
                onFinished={(result) => {
                  setResults(task.id, result)
                }}
                onError={(error) => {
                  setErrors(task.id, error)
                }}
              >
...
<CardGroup cols={2}>
  ...
  <Card title="Collect results with Solid Store" icon="database">
...
<CardGroup cols={2}>
  ...
  <Card title="State Management" icon="database" href="/concepts/state-management">
    Best practices for managing parallel state
  </Card>
</CardGroup>
