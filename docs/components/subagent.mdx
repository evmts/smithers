...
```tsx
import { Claude, Subagent } from 'smithers'
...
1. The first sequential `<Claude>` node (if any)
2. All `<Subagent>` nodes in parallel
...
<>
  ...
  <Subagent name="a">
    <Claude>Parallel A</Claude>     {/* Runs with Sequential 1 */}
  </Subagent>

  <Subagent name="b">
    <Claude>Parallel B</Claude>     {/* Runs with Sequential 1 */}
  </Subagent>
</>
```
