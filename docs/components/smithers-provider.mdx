...
<ParamField path="config" type="SmithersConfig">
  ...
  **SmithersConfig properties:**
  ...
  - `maxIterations`: Default max iterations for Ralph/While loops (default: 10). Individual loops can override via `maxIterations` prop.
...
function WorkflowContent() {
  ...
  return (
    <Ralph 
      ...
      maxIterations={10}
    >
      <Phase name="research" active={currentPhase === "research"}>
        <Claude onFinished={() => setPhase("implement")}>
          Research the problem space.
        </Claude>
      </Phase>

      <Phase name="implement" active={currentPhase === "implement"}>
