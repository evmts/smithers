...
<Note>
Sequential Steps before/after Parallel sequence when Parallel blocks are direct Phase children.
Nested Parallel inside a Step remains scoped to that Step. See [Known Limitations](#known-limitations).
</Note>
...
- Parallel uses a nested step registry; when used directly under a Phase it registers as a single step group
- Nested Parallel inside a Step remains scoped to that Step
- Completion still depends on step task tracking; long-running unrelated tasks can delay Parallel completion
</Warning>
