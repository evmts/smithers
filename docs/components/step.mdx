...
```tsx
function MultiStepAgent(props) {
  return (
    <Claude tools={[filesystem]}>
      <Phase name="setup">
        <Step completed={props.completedSteps.includes('install')}>
          Install dependencies
        </Step>
        <Step completed={props.completedSteps.includes('config')}>
          Configure environment
        </Step>
        <Step completed={props.completedSteps.includes('validate')}>
          Validate setup
        </Step>
      </Phase>
    </Claude>
...
```tsx
function GitCommitSteps(props) {
  return (
    <>
      <Step>Stage all modified files</Step>
      <Step>Review the diff</Step>
      <Step>Commit with message: "{props.message}"</Step>
    </>
  )
}

function TestingSteps(props) {
  return (
    <>
      <Step>Run unit tests</Step>
      <Step>Check coverage is at least {props.coverage}%</Step>
