---
title: Database
description: SQLite persistence for state and history
---

# Database

All state lives in SQLite. Survives restarts. Enables replay.

```tsx
const db = createSmithersDB({ path: ".smithers/data.db" });

// State
db.state.set("phase", "review");
db.state.get("phase");

// Execution
db.execution.start("Task", "task.tsx");
db.execution.findIncomplete();

// Always close
db.close();
```

## Creating a Database

```tsx
import { createSmithersDB } from "smithers-orchestrator";

const db = createSmithersDB({
  path: ".smithers/my-workflow.db"
});

// Always close when done
db.close();
```

<Note>
**Path semantics:** The `path` parameter specifies a SQLite **file** path. For manual queries, use `sqlite3 .smithers/my-workflow.db`.
</Note>

## Database Structure

The database tracks several entity types:

| Entity | Purpose |
|--------|---------|
| **Executions** | Top-level workflow runs |
| **Phases** | Workflow phases within executions |
| **Steps** | Individual steps within phases |
| **Agents** | Claude invocations with prompts and results |
| **Tool Calls** | Tools invoked by agents |
| **State** | Key-value workflow state |
| **Memories** | Long-term knowledge across sessions |
| **VCS** | Git commits, snapshots, and reviews |

## Execution Tracking

Every workflow run is an execution:

```tsx
// Start a new execution
const executionId = db.execution.start(
  "Feature Implementation",  // name
  "scripts/feature.tsx"      // file path
);

// Complete the execution
db.execution.complete(executionId, {
  summary: "Feature implemented successfully"
});

// Or mark as failed
db.execution.fail(executionId, "Tests failed");

// Find incomplete executions (for resume)
const incomplete = db.execution.findIncomplete();
if (incomplete) {
  console.log(`Resuming execution: ${incomplete.id}`);
}
```

## State Management

The state API provides key-value storage with history:

```tsx
// Set a value
db.state.set("phase", "review");

// Set with trigger (for tracking what caused the change)
db.state.set("phase", "review", "tests_passed");

// Get a value
const phase = db.state.get("phase");

// Get all state
const all = db.state.getAll();
// { phase: "review", lastAgent: "abc123", ... }

// Set multiple values atomically
db.state.setMany({
  phase: "complete",
  completedAt: Date.now(),
});
```

### State History

Every state change is recorded:

```tsx
// Get history for a key
const history = db.state.history("phase", 10);
// [
//   { id: "t1", key: "phase", value: "start", trigger: "init", created_at: ... },
//   { id: "t2", key: "phase", value: "review", trigger: "tests_passed", ... },
// ]

// Get all transitions (no key filter)
const allHistory = db.state.history(undefined, 100);
```

## Agent Tracking

Track Claude invocations:

```tsx
// Start an agent
const agentId = db.agents.start(
  "Implement the feature",     // prompt
  "sonnet",                     // model
  "You are a senior engineer"   // system prompt
);

// Complete with results
db.agents.complete(
  agentId,
  "Feature implemented",        // output
  { success: true },            // structured result
  { input: 1000, output: 500 }  // tokens
);

// List agents in current execution
const agents = db.agents.list(executionId);
```

## Tool Call Tracking

Track tools used by agents:

```tsx
// Start a tool call
const toolId = db.tools.start(
  agentId,
  "Edit",
  { file: "src/main.ts", changes: "..." }
);

// Complete with output
db.tools.complete(
  toolId,
  "File edited successfully",
  "Edited src/main.ts: added 15 lines"
);

// List tool calls for an agent
const tools = db.tools.list(agentId);
```

## Task Tracking

Track async work so the Ralph loop knows when to advance:

```tsx
// Start a task (returns task ID)
const taskId = db.tasks.start("fetch-data", "api-call");

try {
  await fetchData();
  db.tasks.complete(taskId);
} catch (err) {
  db.tasks.fail(taskId, err.message);
}

// Query running tasks
const running = db.tasks.listRunning(executionId);
```

The orchestrator waits for all tasks to complete before advancing to the next iteration.

## Memories (Long-term Knowledge)

Store knowledge that persists across workflows:

```tsx
// Add a fact
db.memories.addFact(
  "api-endpoint",
  "The API endpoint is at /api/v2/users",
  "discovered in auth.ts"
);

// Add a learning
db.memories.addLearning(
  "test-pattern",
  "Use vitest for unit tests in this project",
  "package.json"
);

// Add a preference
db.memories.addPreference(
  "commit-style",
  "Use conventional commits with scope"
);

// Search memories
const relevant = db.memories.search("API endpoint");

// Get specific memory
const memory = db.memories.get("fact", "api-endpoint");
```

### Memory Categories

| Category | Use Case |
|----------|----------|
| `fact` | Discovered facts about the codebase |
| `learning` | Lessons learned from past executions |
| `preference` | User preferences and conventions |
| `context` | Contextual information |
| `skill` | Reusable skills or patterns |

### Memory Scopes

| Scope | Visibility |
|-------|------------|
| `global` | All projects and sessions |
| `project` | Current project only |
| `session` | Current session only |

## VCS Integration

Track version control operations:

```tsx
// Log a commit
db.vcs.logCommit({
  hash: "abc123",
  message: "feat: Add user auth",
  author: "claude",
  timestamp: new Date(),
  filesChanged: 3,
  insertions: 150,
  deletions: 20,
  vcsType: "git",
});

// Log a snapshot (jj)
db.vcs.logSnapshot({
  snapshotId: "xyz789",
  description: "Before refactoring",
  timestamp: new Date(),
});

// Log a review
db.vcs.logReview({
  reviewId: "r1",
  target: "abc123",
  approved: false,
  summary: "Security issues found",
  issues: [
    { severity: "high", file: "auth.ts", description: "..." }
  ],
});
```

## Direct Queries

For advanced use cases, query the database directly:

```tsx
// Run a custom query
const results = db.db.query<{ count: number }, [string]>(
  "SELECT COUNT(*) as count FROM agents WHERE model = ?"
).all("sonnet");
console.log(`${results[0].count} sonnet agents`);
```

## Inspecting the Database

Use the CLI to inspect database contents:

```bash
# List executions
smithers db executions

# View current state
smithers db state

# View database statistics
smithers db stats

# View current execution
smithers db current
```

## Best Practices

<AccordionGroup>
  <Accordion title="Always close the database">
    Ensure the database is closed to flush writes:
    ```tsx
    const db = createSmithersDB({ path: ".smithers/data.db" });
    try {
      // ... do work
    } finally {
      db.close();
    }
    ```
  </Accordion>

  <Accordion title="Use triggers for debugging">
    Set triggers when updating state to track causality:
    ```tsx
    db.state.set("phase", "review", "tests_passed");
    // Later, history shows why the change happened
    ```
  </Accordion>

  <Accordion title="Check for incomplete executions on startup">
    Resume interrupted workflows:
    ```tsx
    const incomplete = db.execution.findIncomplete();
    if (incomplete) {
      // Resume the workflow
      const state = db.state.getAll();
      // ... continue from saved state
    }
    ```
  </Accordion>
</AccordionGroup>

## Next Steps

<CardGroup cols={2}>
  <Card title="State API Reference" icon="code" href="/api-reference/state">
    Complete state management API
  </Card>
  <Card title="Memories API Reference" icon="brain" href="/api-reference/memories">
    Long-term knowledge storage
  </Card>
</CardGroup>
