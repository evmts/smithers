...
Each worktree operates independently, preventing conflicts on restart.
...
<AccordionGroup>
  ...
  <Accordion title="Use SMITHERS_EXECUTION_ID for deterministic IDs">
    ...
    ```bash
    SMITHERS_EXECUTION_ID=job-123 bun run workflow.tsx
    ```
