...
|----------|------|-------------|
| `Provider` | `Component` | Context provider for the workflow |
| `useInput` | `(name) => () => T \| undefined` | Hook to subscribe to a workflow value (returns accessor) |
| `useStore` | `() => WorkflowStore` | Hook to access the full store |
| `Output` | `Component` | Component to define agent outputs |
...
function Writer() {
  ...
  return () => {
    ...
    return (
      <Claude>
        Summarize the following research: {r}
...
3. The workflow store updates, triggering a signal update
4. `<Writer />`'s effect re-runs because `research()` changed
...
2. The component re-executes the closure (fine-grained update)
3. The Claude node's children contain different text
4. Content hash differs from previous execution
5. Ralph loop marks the node for re-execution
...
const count = workflow.useInput('count') // accessor: () => number | undefined
const name = workflow.useInput('name')   // accessor: () => string | undefined
...
| Feature | Signals/Store | Workflows |
|---------|------------------|-----------|
...
Use Signals/Store when you want:
