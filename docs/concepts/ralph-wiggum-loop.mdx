...
┌─────────────────────────────────────────────────────────────┐
│  4. UPDATE                                                  │
│     onFinished callbacks update SolidJS signals             │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│  5. RE-RENDER                                               │
...
<AccordionGroup>
  <Accordion title="Simple mental model">
    You don't need to think about complex orchestration. Write SolidJS components with conditional rendering, and the loop handles execution.
  </Accordion>
    ...
    if (p === 'research') {
      return (
        <Claude onFinished={(data) => {
          ...
          Research {props.topic}. Find key facts.
        </Claude>
      )
    }
...
function ParallelResearch(props) {
  ...
  return () => {
    const pending = props.topics.filter(t => !results[t])
...
<CardGroup cols={2}>
  <Card title="State Management" icon="database" href="/concepts/state-management">
    Learn SolidJS patterns for managing agent state
  </Card>
