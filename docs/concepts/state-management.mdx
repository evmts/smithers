...
For complex nested state, use `useState` with object spreading:

```tsx
import { useState } from "react";

function WorkflowWithComplexState() {
    ...
    <Ralph maxIterations={10}>
      {state.phase === "research" && (
        <Claude onFinished={(result) => {
          setState(prev => ({
            ...prev,
            findings: [...prev.findings, result.output],
            phase: "analyze",
          }));
        }}>
          Research the topic.
      ...
      {state.phase === "analyze" && (
        <Claude onFinished={() => {
          setState(prev => ({
            ...
            metadata: {
              ...prev.metadata,
              iterations: prev.metadata.iterations + 1,
            },
          }));
        }}>
...
setState(prev => ({
  ...prev,
  metadata: { ...prev.metadata, iterations: 5 },
}));

// Update with function (increment)
setState(prev => ({
  ...prev,
  metadata: { ...prev.metadata, iterations: prev.metadata.iterations + 1 },
}));

// Update array
setState(prev => ({
  ...prev,
  findings: [...prev.findings, "new finding"],
}));
...
setState(prev => ({
  ...prev,
  phase: "done",
  metadata: { ...prev.metadata, completedAt: Date.now() },
}));
```
...
async function ResumeableWorkflow() {
  ...
  const persistState = useCallback(async (updates: Partial<typeof state>) => {
...
<AccordionGroup>
  ...
  <Accordion title="Group related state together">
    Use a single state object for related values:
    ```tsx
    // Prefer grouped state for related values
    const [state, setState] = useState({
      phase: "start",
      data: { items: [], metadata: {} },
    });

    // Updating grouped state
    setState(prev => ({
      ...prev,
      phase: "next",
    ...
    // Multiple separate state variables can be harder to keep in sync
    const [phase, setPhase] = useState("start");
