...
[Full skill content - see plugins/skill/SKILL.md for complete version]
```
...
```bash
bunx smithers-orchestrator serve --port 3847
```
...
```json
{
  "mcpServers": {
    "smithers": {
      "command": "bunx",
      "args": ["smithers-orchestrator", "serve"],
      "env": {
...
# With options
bunx smithers run workflow.tsx --resume --verbose

# Database commands
bunx smithers db executions
bunx smithers db state --execution-id <id>
bunx smithers db stats
```
