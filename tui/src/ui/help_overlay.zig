...
pub const HelpOverlay = struct {
    ...
    pub fn draw(self: *const Self, win: vaxis.Window) void {
        ...
        const bg_style: vaxis.Style = .{ .bg = .{ .index = bg_color }, .fg = .{ .index = desc_color } };
        const border_style: vaxis.Style = .{ .bg = .{ .index = bg_color }, .fg = .{ .index = border_color } };
        const key_style: vaxis.Style = .{ .bg = .{ .index = bg_color }, .fg = .{ .index = key_color }, .bold = true };
        const header_style: vaxis.Style = .{ .bg = .{ .index = bg_color }, .fg = .{ .index = header_color }, .bold = true };
    ...
    fn drawBinding(_: *const Self, win: vaxis.Window, bx: u16, by: u16, key: []const u8, desc: []const u8, key_style: vaxis.Style, desc_style: vaxis.Style) void {
