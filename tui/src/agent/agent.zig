...
test "Agent init" {
    ...
    try std.testing.expect(!agent.hasProvider());
...
test "Agent tool registry initialized" {
    ...
    try std.testing.expect(agent.tool_registry.get("bash") != null);
    try std.testing.expect(agent.tool_registry.get("read_file") != null);
    try std.testing.expect(agent.tool_registry.get("write_file") != null);
}
...
test "Agent clearHistory" {
    ...
    try agent.context.messages.append(allocator, Message.user("test"));
    try std.testing.expectEqual(@as(usize, 1), agent.messageCount());
