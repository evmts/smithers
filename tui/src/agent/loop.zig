...
pub const AgentLoop = struct {
    ...
    pub fn init(alloc: std.mem.Allocator, loading: *loading_mod.LoadingState) AgentLoop {
        return .{
            .alloc = alloc,
            .loading = loading,
        };
    ...
    pub fn tick(self: *AgentLoop, database: *db.DefaultDatabase, chat_history: *ChatHistory) !bool {
        ...
        if (self.loading.pending_query != null and self.loading.streaming == null and self.loading.start_time > 0) {
        ...
        if (self.loading.pending_continuation != null and self.loading.streaming == null and self.loading.start_time > 0) {
            const elapsed = std.time.milliTimestamp() - self.loading.start_time;
            if (elapsed >= 50) {
        ...
        if (self.loading.streaming != null) {
    ...
    fn start_query_stream(self: *AgentLoop, database: *db.DefaultDatabase, chat_history: *ChatHistory) !bool {
        ...
        self.loading.streaming = streaming.StreamingState.init(self.alloc);
        self.loading.streaming.?.message_id = msg_id;
        self.loading.streaming.?.startStream(api_key, request_body) catch |err| {
    ...
    fn poll_active_stream(self: *AgentLoop, database: *db.DefaultDatabase, chat_history: *ChatHistory) !void {
        ...
        if (is_done) {
            ...
            if (stream.hasToolCalls()) {
                ...
                if (self.loading.streaming) |*s| s.cleanup();
                self.loading.streaming = null;
            } else {
                self.loading.cleanup(self.alloc);
            }
        }
