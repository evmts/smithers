...
test "visibleWidth multiple ANSI sequences" {
    const allocator = std.testing.allocator;
    const width = visibleWidthWithAllocator(allocator, "\x1b[1m\x1b[31mBold Red\x1b[0m");
    try std.testing.expectEqual(@as(u32, 8), width);
}

test "visibleWidth Hiragana" {
    const width = calculateWidth("こんにちは");
    try std.testing.expectEqual(@as(u32, 10), width);
}

test "visibleWidth Katakana" {
    const width = calculateWidth("カタカナ");
    try std.testing.expectEqual(@as(u32, 8), width);
}

test "visibleWidth Hangul" {
    const width = calculateWidth("한글");
    try std.testing.expectEqual(@as(u32, 4), width);
}
