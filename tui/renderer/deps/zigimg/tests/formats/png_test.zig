...
test "png: Don't write tRNS chunk in indexed format when there is no alpha" {
    ...
    defer {
        std.Io.Dir.cwd().deleteFile(image_file_name) catch {};
    }
...
pub fn testWithDir(directory: []const u8, test_md5_signature: bool) !void {
    var test_dir_opt = std.Io.Dir.cwd().openDir(directory, .{ .access_sub_paths = false, .no_follow = true, .iterate = true }) catch null;
