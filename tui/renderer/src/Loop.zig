            ...
            switch (builtin.os.tag) {
                .windows => {
                    ...
                    read_loop: while (!self.should_quit) {
                        const n = self.tty.read(buf[read_start..]) catch |err| switch (err) {
                            error.NotOpenForReading => return, // TTY closed during shutdown
                            else => return err,
                        };
                        if (n == 0) continue :read_loop;
                        log.debug("ttyRun: read {d} bytes", .{n});
                        ...
                        while (seq_start < n) {
                            ...
                            log.debug("ttyRun: got event, posting to queue", .{});
