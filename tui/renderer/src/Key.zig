...
/// Inline storage for text (max 16 bytes covers most grapheme clusters)
text_buf: [16]u8 = undefined,
text_len: u8 = 0,

/// the text generated from the key event. DEPRECATED - use getText() instead.
/// The underlying slice has a limited lifetime.
text: ?[]const u8 = null,
...
/// Get the text if any (from inline buffer)
pub fn getText(self: *const Key) ?[]const u8 {
    if (self.text_len == 0) return null;
    return self.text_buf[0..self.text_len];
}

/// Set the text (copies into inline buffer)
pub fn setText(self: *Key, t: []const u8) void {
    const len = @min(t.len, self.text_buf.len);
    @memcpy(self.text_buf[0..len], t[0..len]);
