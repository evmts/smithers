...
test.describe('Agent Tool: read_file', () => {
  test('reads package.json and shows content', async ({ terminal }) => {
    ...
    await expect(terminal.getByText('smithers', { full: true })).toBeVisible({ timeout: 30000 })
  })
  ...
  test('reads file with line range', async ({ terminal }) => {
    ...
    await expect(terminal.getByText(/README|Smithers|#/i, { full: true })).toBeVisible({ timeout: 30000 })
  })
  ...
  test('reads zig source file', async ({ terminal }) => {
    ...
    await expect(terminal.getByText(/import|std|vaxis/i, { full: true })).toBeVisible({ timeout: 30000 })
  })
})
