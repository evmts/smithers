...
test.describe('Agent Multi-Tool Integration', () => {
  ...
  test('creates file then verifies with read', async ({ terminal }) => {
    ...
    await expect(terminal.getByText('INTEGRATION_TEST_123', { full: true, strict: false })).toBeVisible({ timeout: 30000 })
...
test.describe('Agent Tool Streaming', () => {
  test('shows streaming output for long bash command', async ({ terminal }) => {
    ...
    await expect(terminal.getByText('.zig', { full: true, strict: false })).toBeVisible({ timeout: 30000 })
  })
  ...
  test('handles tool cancellation', async ({ terminal }) => {
    ...
    await expect(terminal.getByText('bash', { full: true, strict: false })).toBeVisible({ timeout: 15000 })
