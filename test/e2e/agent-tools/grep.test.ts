...
test.describe('Agent Tool: grep', () => {
  ...
  test('searches with regex pattern', async ({ terminal }) => {
    ...
    await expect(terminal.getByText('grep', { full: true, strict: false })).toBeVisible({ timeout: 30000 })
    // Should find imports
    await expect(terminal.getByText('@import', { full: true, strict: false })).toBeVisible({ timeout: 30000 })
  })
  ...
  test('searches in directory recursively', async ({ terminal }) => {
    ...
    await expect(terminal.getByText('ToolResult', { full: true, strict: false })).toBeVisible({ timeout: 30000 })
  })
