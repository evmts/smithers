...
**P0 `useQuery` invalidation bug** - FIXED
- `src/reactive-sqlite/hooks/useQuery.ts` line 111 now calls `invalidateCache()` in the subscription callback
- Cache is properly cleared before `onStoreChange()`, so `getSnapshot()` recomputes and returns new snapshot

**P1 `REPLACE INTO` support** - FIXED  
- `src/reactive-sqlite/parser.ts` lines 71-75 now include `replaceRegex` pattern
- `REPLACE INTO table_name` statements properly invalidate subscriptions

### Remaining Items (Not Blocking)

**Transaction-aware invalidation** - NOT IMPLEMENTED
- Invalidations still fire mid-transaction (before commit/rollback)
- Could cause spurious rerenders if transaction rolls back
- Acceptable if Smithers doesn't rely on transactional atomicity for UI updates

**Other P1/P2 items** from original review (hook ergonomics, parser edge cases, reentrancy hardening) not verified but lower priority
