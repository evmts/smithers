-- Smithers Orchestrator Database Schema (Python Port)
...
CREATE TABLE IF NOT EXISTS render_frames (
  ...
  -- Frame content
  xml_content TEXT NOT NULL,        -- Serialized node tree as XML (renamed from tree_xml)
  ralph_count INTEGER NOT NULL DEFAULT 0,  -- Current Ralph iteration count

  -- Timing
  timestamp TEXT DEFAULT (datetime('now')),  -- Renamed from created_at
