...
pub const AgentProvider = struct {
    ...
    pub fn init(allocator: Allocator, interface: ProviderInterface, config: ProviderConfig) Self {
        return .{
            .interface = interface,
            .config = config,
            .allocator = allocator,
...
pub fn createMockWithResponse(allocator: Allocator, response: []const u8) !MockProvider {
    ...
    try provider.addResponse(.{ .type = .text_delta, .delta = response });
...
test "createMockWithResponse" {
    ...
    var stream = try mock.interface().stream(model, &ctx, .{}, allocator);
    defer stream.deinit();

    _ = stream.next(); // start
    const text_ev = stream.next();
