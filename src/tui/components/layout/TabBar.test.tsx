...
import type { TabInfo, TabKey } from '../../App.js'
...
describe('tui/components/layout/TabBar', () => {
  ...
  describe('active tab styling', () => {
    test('applies blue (#7aa2f7) to active tab label', () => {
      const props = createProps({ activeTab: 'timeline' })
      const element = TabBar(props)
      
      const firstTabBox = element.props.children[0]
      const labelText = firstTabBox.props.children[1]
      expect(labelText.props.style.fg).toBe('#7aa2f7')
    })

    test('applies gray (#a9b1d6) to inactive tab labels', () => {
      const props = createProps({ activeTab: 'timeline' })
      const element = TabBar(props)
      ...
      expect(labelText.props.style.fg).toBe('#a9b1d6')
    })

    test('applies background (#24283b) to active tab', () => {
      ...
      const secondTabBox = element.props.children[1]
      const labelText = secondTabBox.props.children[1]
      expect(labelText.props.style.backgroundColor).toBe('#24283b')
    })
    ...
    test('applies bold to active tab label', () => {
      ...
      expect(labelText.props.style.attributes).toBe(1)
    })
  ...
  describe('shortcut styling', () => {
    test('shortcut always uses gray (#565f89)', () => {
      const props = createProps()
      const element = TabBar(props)
      
      const firstTabBox = element.props.children[0]
      const shortcutText = firstTabBox.props.children[0]
      expect(shortcutText.props.style.fg).toBe('#565f89')
    })
    ...
    test('shortcut color unchanged by active state', () => {
      ...
      expect(activeShortcut.props.style.fg).toBe('#565f89')
      ...
      expect(inactiveShortcut.props.style.fg).toBe('#565f89')
    })
  })
  ...
  describe('props', () => {
    ...
    test('receives onTabChange callback (currently unused)', () => {
      ...
      // Currently the component doesn't use onTabChange, but it accepts it
      const element = TabBar(props)
