...
const REVIEW_OUTPUT_PATH = path.join(OUTPUT_DIR, 'pr-review.md')
...
*Generated by Smithers AI Review*`
  ...
  ensureOutputDir()
  await Bun.write(REVIEW_OUTPUT_PATH, body)

  if (!PR_NUMBER || PR_NUMBER.trim() === '') {
    console.log('Skipping PR comment: missing PR_NUMBER')
    return
  }

  if (!hasGithubToken()) {
    console.log(`Skipping PR comment: no GitHub token (wrote ${REVIEW_OUTPUT_PATH})`)
    return
  }

  await Bun.$`gh pr comment ${PR_NUMBER} --body-file ${REVIEW_OUTPUT_PATH}`.quiet()
}
