...
import { Claude, Phase, Step, Persona } from 'smithers'
import { fileSystem, runTests } from './tools'
...
export function Developer({ subtask, onComplete }) {
  return (
    <Claude tools={[fileSystem]} onFinished={onComplete}>
...
export function Reviewer({ onApprove }) {
  return (
    <Claude tools={[fileSystem, runTests]} onFinished={onApprove}>
