...
export function MergeOrderPhase({ candidates, onOrderDetermined }: MergeOrderPhaseProps): ReactNode {
    ...
    <phase-content>
      ...
      <Step name="validate-order">
        <Claude model="sonnet">
          ...
          {candidates.map((c, i) => `${i + 1}. PR #${c.pr.number} - ${c.pr.title} (+${c.pr.additions}/-${c.pr.deletions}, ${c.pr.changedFiles} files)`).join('\n')}

          Analyze if this order makes sense:
          1. Are there any obvious dependencies where one PR should come before another?
          2. Are there any PRs that touch the same files and might conflict?
          3. Should any PRs be merged together or split?

          If the order looks good, respond with "ORDER_APPROVED".
          If changes are needed, explain the recommended order.
        </Claude>
      </Step>
    </phase-content>
  )
